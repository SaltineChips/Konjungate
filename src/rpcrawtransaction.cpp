...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    set<CKonjungateAddress> setAddress;
    if (params.size() > 2)
    {
        Array inputs = params[2].get_array();
        BOOST_FOREACH(Value& input, inputs)
        {
            CKonjungateAddress address(input.get_str());
            if (!address.IsValid())
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address))
        {
            entry.push_back(Pair("address", CKonjungateAddress(address).ToString()));
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        CKonjungateAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Konjungate address: ")+s.name_);
...
Value decodescript(const Array& params, bool fHelp)
{
    ...
    r.push_back(Pair("p2sh", CKonjungateAddress(script.GetID()).ToString()));
    return r;
}
